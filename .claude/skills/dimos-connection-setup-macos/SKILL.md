...
- 已知风险：`error_code 0x4` 出现过多次（joint1 一次：保持 ~1 分钟后 fault；joint2 两次：均在移到高举/前伸位姿并保持后 fault，2026-07-25）。模式指向**持重保持触发电机保护**（过流/过热），而非通讯问题——高举位姿下 j2 重力力矩大（实测保持时 eff ≈ -7.5 Nm）。对策：高举位姿尽量缩短保持时间、避免长时间 hold，连续测试间让电机休息

## MCP / blueprint 开关机安全规则（强制）

1. **启动 MCP server 前，机械臂必须已在零位**（全关节 0°）。`robot.start()` 会立即锁死当前位姿，从随意姿态上电会让后续所有运动从不可预期的位置开始。
2. **关闭 MCP server 之前，必须先调用安全关机 tool，让臂先回到零位再下电**；绝不允许在高举/前伸位姿直接断电或直接杀进程（电机失能 = 臂立刻变软坠落）。
3. **凡是写 blueprint / Module 暴露机械臂 MCP tool，必须同时暴露一个"安全位置关机" tool**，行为固定为：低速回零位 → 刚度/重力补偿斜坡到零 → 失能电机。参考实现：`a1z/a1z_mcp_server.py` 的 `shutdown`（先 `move_joints(zeros, speed=0.2)` 驻停零位，再软释放）。`estop` 类立即失能的 tool 只能作为急停兜底，不能当正常退出路径。
